...
export function Synth(voiceClass, polyphony) {
  this.voices = [];
...
Synth.prototype.pitchBend = function (value) {
  ...
  for (var i = 0, l = this.voices.length; i < l; i++) {
    if (this.voices[i]) this.voices[i].updatePitchBend();
  }
};
...
Synth.prototype.noteOff = function (note) {
  for (
    var i = 0, voice;
    i < this.voices.length, (voice = this.voices[i]);
    i++
  ) {
    if (voice && voice.note === note && voice.down === true) {
      voice.down = false;
      if (this.sustainPedalDown === false) voice.noteOff();
      break;
    }
  }
};
...
Synth.prototype.panic = function () {
  this.sustainPedalDown = false;
  for (var i = 0, l = this.voices.length; i < l; i++) {
    if (this.voices[i]) this.voices[i].noteOff();
  }
  this.voices = [];
};
