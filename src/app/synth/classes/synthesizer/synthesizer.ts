...
export class Synthesizer {
  ...
  constructor(
    ...
    this.audioContext = new ((window as any).AudioContext ||
      (window as any).webkitAudioContext)({ sampleRate: 88200 });
  ...
  timeUpdate() {
    ...
    this.currentBeat['%'] =
      ((this.currentTime / this.currentTempo / 60) * 4 * 100) % 100;
    const sixteenFourth = Math.floor(
      ((this.currentTime / 60) * this.currentTempo) / 64
    );
